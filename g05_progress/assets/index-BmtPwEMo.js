...
`;var _=document.getElementById(`chapter-cards`);i.forEach(e=>{let t=document.createElement(`div`);t.id=`card-${e.id}`,t.className=`ch-card glass rounded-xl p-4 ch-locked`,t.style.setProperty(`--ch-color`,e.color),t.innerHTML=`
  ...
  `,k.appendChild(t)});function A(){u.forEach(e=>{let t=document.getElementById(`fbadge-${e.id}`),n=document.getElementById(`frow-${e.id}`),r=n.querySelector(`.feat-title`);if(m<e.unlockAt)n.classList.add(`ch-locked`),t.textContent=`🔒`,r.style.color=``,n.style.borderColor=`${l}15`;else{let i=e.optional?`#4ade80`:l;n.classList.remove(`ch-locked`),t.textContent=`✔`,t.style.color=i,r.style.color=i,n.style.borderColor=`${i}40`}})}function j(){n=e*t,y.setAttribute(`aria-valuemax`,n);let r=document.getElementById(`seg-container`);r.innerHTML=Array.from({length:e},(e,t)=>`
    ...
    <div style="position:relative;height:14px;border-radius:8px;background:rgba(255,255,255,0.08);margin-bottom:10px" id="th-track">
      <div id="th-fill" style="position:absolute;left:0;top:0;height:100%;border-radius:8px;background:${e};width:10%;pointer-events:none"></div>
      <div id="th-overlay" style="position:absolute;inset:0;cursor:pointer;border-radius:8px"></div>
      <div id="th-handle" style="position:absolute;top:50%;transform:translate(-50%,-50%);left:10%;width:20px;height:20px;border-radius:50%;background:${e};border:2px solid rgba(255,255,255,0.8);pointer-events:none"></div>
    </div>
    <div id="th-ticks" style="position:relative;height:20px;margin-top:2px;margin-bottom:4px"></div>
    <p class="text-sm" style="color:${e}" id="th-display"><b>Lv.1</b> / 10</p>
    <div id="th-unlock-info" class="mt-4" style="min-height:44px"></div>
  `,N.appendChild(t)}function I(){P=Math.max(0,Math.min(P,F));let e=F>0?P/F*100:0;document.getElementById(`th-handle`).style.left=`${e}%`,document.getElementById(`th-fill`).style.width=`${e}%`,document.getElementById(`th-display`).innerHTML=`<b>Lv.${P}</b> / ${F}`;let t=document.getElementById(`th-current`);document.activeElement!==t&&(t.value=P);let n=document.getElementById(`th-ticks`);n.innerHTML=``;for(let e=0;e<=F;e++){let t=e/F*100,r=e===0||e===F||e%5==0,i=document.createElement(`div`);if(i.style.cssText=`position:absolute;left:${t}%;transform:translateX(-50%);width:${r?2:1}px;height:${r?8:5}px;background:rgba(255,255,255,${r?.4:.18});top:0`,n.appendChild(i),r){let r=document.createElement(`span`);r.style.cssText=`position:absolute;left:${t}%;transform:translateX(-50%);font-size:10px;color:rgba(255,255,255,0.3);top:9px;white-space:nowrap;line-height:1`,r.textContent=e,n.appendChild(r)}}R()}function L(e){if(e<=0)return 0;let t=1;for(let n=0;n<i.length&&e>=i[n].start;n++)t=n+1;return t}function R(){let e=document.getElementById(`th-unlock-info`);if(!e)return;let t=P,n=(e,t)=>`<div style="background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.13);border-radius:8px;padding:6px 14px;font-size:13px;font-weight:600;color:${t};white-space:nowrap">${e}</div>`,r=[];if(t>=2&&i[t-2]){let e=i[t-2],a=e.end-e.start,o=Math.round(a*60/50);r.push(n(`关卡${e.id}-${o}`,`rgba(255,255,255,0.75)`))}t>=2&&(r.push(n(`医馆 Lv.${(t-1)*10}`,a)),r.push(n(`熔铸所 Lv.${t*10}`,a))),t>=3&&r.push(n(`製皮厂 Lv.${t*10}`,a)),t>=4&&r.push(n(`晶石矿场 Lv.${t*10}`,a));let o=[];c.forEach(e=>{if(e.id!==`town_hall`&&L(e.unlockAt)===t&&t>=1){let t=e.noLevel?s:a;o.push(n(e.name,t))}}),u.forEach(e=>{if(L(e.unlockAt)===t){let t=e.optional?`#4ade80`:l;o.push(n(e.name+(e.optional?` (可选)`:``),t))}});let d=[`医馆`,`熔铸所`];t>=3&&d.push(`製皮厂`),t>=4&&d.push(`晶石矿场`),t>=2&&o.push(n(`升级建筑等级上限 → Lv.${t*10}`,a));let f=``;r.length>0&&(f+=`<p class="text-white/30 text-xs font-semibold uppercase tracking-widest mb-2">需求</p>
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px">${r.join(``)}</div>`),o.length>0&&(f+=`<p class="text-white/30 text-xs font-semibold uppercase tracking-widest mb-2">解锁</p>
    <div style="display:flex;flex-wrap:wrap;gap:8px">${o.join(``)}</div>`),e.innerHTML=f}function z(){let e=document.getElementById(`th-track`),t=document.getElementById(`th-overlay`),n=!1,r=t=>{let n=e.getBoundingClientRect(),r=t.touches?t.touches[0].clientX:t.clientX;return Math.max(0,Math.min(1,(r-n.left)/n.width))},i=e=>{P=Math.round(e*F),I()};t.addEventListener(`mousedown`,e=>{n=!0,i(r(e)),e.preventDefault()}),t.addEventListener(`touchstart`,e=>{n=!0,i(r(e)),e.preventDefault()},{passive:!1}),document.addEventListener(`mousemove`,e=>{n&&i(r(e))}),document.addEventListener(`touchmove`,e=>{n&&(i(r(e)),e.preventDefault())},{passive:!1}),document.addEventListener(`mouseup`,()=>{n=!1}),document.addEventListener(`touchend`,()=>{n=!1}),document.getElementById(`th-current`).addEventListener(`input`,e=>{let t=parseInt(e.target.value);!isNaN(t)&&t>=0&&(P=t,I())}),document.getElementById(`th-max`).addEventListener(`input`,e=>{let t=parseInt(e.target.value);!isNaN(t)&&t>=1&&(F=t,I())}),I()}z();
...
`;var m=document.getElementById(`chapter-cards`);i.forEach(e=>{let t=document.createElement(`div`);t.id=`card-${e.id}`,t.className=`ch-card glass rounded-xl p-4`,t.style.setProperty(`--ch-color`,e.color),t.innerHTML=`
  ...
  `,C.appendChild(t)});function w(){s.forEach(e=>{let t=document.getElementById(`bbadge-${e.id}`),n=document.getElementById(`bbar-${e.id}`),r=document.getElementById(`brow-${e.id}`),i=r.querySelector(`.bldg-name`);if(d<e.unlockAt){r.classList.add(`bldg-locked`),r.classList.remove(`bldg-active`),t.textContent=`🔒`,i.style.color=``,n.style.width=`0%`;return}r.classList.remove(`bldg-locked`),r.classList.add(`bldg-active`),i.style.color=o;let s=d-e.unlockAt,c=Math.floor(s/a)+1,l=Math.round(s%a/a*100);t.textContent=`Lv.${c}`,t.style.color=o,n.style.width=`${l}%`})}b(0);
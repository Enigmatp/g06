...
`;var s=document.getElementById(`axis-labels`),c=document.getElementById(`milestone-markers`);t.forEach(t=>{let n=t.minute/e*100,r=document.createElement(`div`);r.className=`milestone-diamond ${t.type}`,r.style.left=`${n}%`,r.title=t.label,c.appendChild(r);let i=document.createElement(`div`);i.className=`axis-label`,i.style.left=`${n}%`,i.innerHTML=`<span class="axis-main">${t.label}</span><span class="axis-sub">${t.desc}</span>`,s.appendChild(i)});function l(e,n){let r=document.getElementById(e);t.filter(e=>e.type===n).forEach(e=>{let t=document.createElement(`div`);t.id=`card-${e.id}`,t.className=`milestone-card glass rounded-xl p-4`,t.innerHTML=`
    ...
    `,r.appendChild(t)})}l(`early-cards`,`early`),l(`daily-cards`,`daily`);var u=document.getElementById(`track`),d=document.getElementById(`handle`),f=document.getElementById(`tooltip`),p=document.getElementById(`tooltip-text`),m=document.getElementById(`time-display`);function h(t){t=n(t,0,1),a=Math.round(t*e);let o=a/e*100;d.style.left=`${o}%`,f.style.left=`${o}%`,d.setAttribute(`aria-valuenow`,a),m.textContent=r(a),p.textContent=i(a),_()}function g(e){let t=u.getBoundingClientRect();return((e.touches?e.touches[0].clientX:e.clientX)-t.left)/t.width}d.addEventListener(`mousedown`,e=>{o=!0,d.classList.add(`dragging`),f.style.display=`block`,e.preventDefault()}),d.addEventListener(`touchstart`,e=>{o=!0,d.classList.add(`dragging`),f.style.display=`block`,e.preventDefault()},{passive:!1}),document.addEventListener(`mousemove`,e=>{o&&h(g(e))}),document.addEventListener(`touchmove`,e=>{o&&(h(g(e)),e.preventDefault())},{passive:!1}),document.addEventListener(`mouseup`,()=>{o&&(o=!1,d.classList.remove(`dragging`),f.style.display=`none`)}),document.addEventListener(`touchend`,()=>{o&&(o=!1,d.classList.remove(`dragging`),f.style.display=`none`)}),u.addEventListener(`click`,e=>{e.target!==d&&h(g(e))}),d.addEventListener(`mouseenter`,()=>f.style.display=`block`),d.addEventListener(`mouseleave`,()=>{o||(f.style.display=`none`)}),d.addEventListener(`keydown`,t=>{let n=1/e,r=a/e;t.key===`ArrowRight`&&(h(r+n),t.preventDefault()),t.key===`ArrowLeft`&&(h(r-n),t.preventDefault()),t.key===`Home`&&(h(0),t.preventDefault()),t.key===`End`&&(h(1),t.preventDefault())});function _(){t.forEach((e,n)=>{let r=document.getElementById(`card-${e.id}`);if(!r)return;let i=r.querySelector(`.card-badge`),o=r.querySelector(`.card-bar`),s=n===0?0:t[n-1].minute;if(a>=e.minute)i.textContent=`已解锁`,i.className=`card-badge unlocked`,o.style.width=`100%`,r.classList.add(`card-active`);else if(a>s){let t=Math.round((a-s)/(e.minute-s)*100);i.textContent=`${t}%`,i.className=`card-badge in-progress`,o.style.width=`${t}%`,r.classList.add(`card-active`)}else i.textContent=`锁定`,i.className=`card-badge`,o.style.width=`0%`,r.classList.remove(`card-active`)})}h(0);
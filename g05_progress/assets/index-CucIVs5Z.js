...
`;var h=document.getElementById(`chapter-cards`);i.forEach(e=>{let t=document.createElement(`div`);t.id=`card-${e.id}`,t.className=`ch-card glass rounded-xl p-4 ch-locked`,t.style.setProperty(`--ch-color`,e.color),t.innerHTML=`
  ...
  `,D.appendChild(t)});function O(){c.forEach(e=>{let t=document.getElementById(`fbadge-${e.id}`),n=document.getElementById(`frow-${e.id}`),r=n.querySelector(`.feat-title`);if(f<e.unlockAt)n.classList.add(`ch-locked`),t.textContent=`🔒`,r.style.color=``,n.style.borderColor=`${s}15`;else{let i=e.optional?`#4ade80`:s;n.classList.remove(`ch-locked`),t.textContent=`✔`,t.style.color=i,r.style.color=i,n.style.borderColor=`${i}40`}})}function k(){n=e*t,_.setAttribute(`aria-valuemax`,n);let r=document.getElementById(`seg-container`);r.innerHTML=Array.from({length:e},(e,t)=>`
    ...
    </div>`}).join(``),x(n>0?l(f/n,0,1):0)}function A(){let n=document.getElementById(`input-days`),r=document.getElementById(`input-mpd`);n.addEventListener(`input`,()=>{let t=parseInt(n.value);!isNaN(t)&&t>=1&&(e=t,k())}),r.addEventListener(`input`,()=>{let e=parseInt(r.value);!isNaN(e)&&e>=1&&(t=e,k())})}k(),A();
...
`;var p=document.getElementById(`chapter-cards`);i.forEach(e=>{let t=document.createElement(`div`);t.id=`card-${e.id}`,t.className=`ch-card glass rounded-xl p-4 ch-locked`,t.style.setProperty(`--ch-color`,e.color),t.innerHTML=`
  ...
  `,S.appendChild(t)});function C(){let e=i.filter(e=>u>=e.start).length-1,t=i[e],n=t.end-t.start,r=n>0?Math.min((u-t.start)/n,1):1;return Math.floor(e*10+r*10)}function w(){let e=i.filter(e=>u>=e.start).length,t=C();o.forEach(n=>{let r=document.getElementById(`bbadge-${n.id}`),o=document.getElementById(`bbar-${n.id}`),s=document.getElementById(`brow-${n.id}`),c=s.querySelector(`.bldg-name`);if(u<n.unlockAt){s.classList.add(`bldg-locked`),s.classList.remove(`bldg-active`),r.textContent=`🔒`,c.style.color=``,o.style.width=`0%`;return}if(s.classList.remove(`bldg-locked`),s.classList.add(`bldg-active`),c.style.color=a,n.id===`town_hall`)r.textContent=`Lv.${e}`,r.style.color=a,o.style.width=`${Math.round(e/i.length*100)}%`;else{let n=e*10,i=Math.min(t,n);r.textContent=`Lv.${i}/${n}`,r.style.color=a,o.style.width=n>0?`${Math.round(i/n*100)}%`:`0%`}})}y(0);
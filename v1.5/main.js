...
const state = {
    ...
    energyRegenTimer: 0,
    cardLevels: {
        angel: 1, archer: 1, fireball: 1, titan: 1, mage: 1, dragon: 1, gaze: 1
    },
...
function createGrid() {
    ...
    // 7x3 = 21 slots, all empty at start
...
function shuffleDeck() {
    for (let i = state.cards.deck.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [state.cards.deck[i], state.cards.deck[j]] = [state.cards.deck[j], state.cards.deck[i]];
    }
}
...
function playCard(index) {
    ...
    // 将使用的卡牌放回抽牌区
    const usedCard = state.cards.hand.splice(index, 1)[0];
    state.cards.deck.push(usedCard);
...
function renderCardHand(playedIndex = -1) {
    ...
    // 显示抽牌区数量
    nextContainer.className = `card-slot next-slot`;
    nextContainer.innerHTML = `
        <div style="font-size: 0.8rem; color: #a1a1aa; margin-bottom: 4px;">抽牌区</div>
        <div style="font-size: 2rem; font-weight: bold;">${state.cards.deck.length}</div>
    `;
    ...
    state.cards.hand.forEach((cardType, idx) => {
        ...
        cardEl.dataset.tooltip = `${config.name}\n消耗: ${cost} ⚡\n${config.desc}`;
        if (state.gold < cost) cardEl.classList.add('unaffordable');

        if (playedIndex !== -1 && idx === state.cards.hand.length - 1) {
        ...
        cardEl.addEventListener('mouseenter', (e) => {
            ...
            state.tooltipTimer = setTimeout(() => {
                showCardTooltip(cardType, cardEl);
            }, 300);
        });
...
function restartGame() {
    ...
    state.costOffset = 0;
    state.regenRate = 1.0;
    state.cardLevels = { angel: 1, archer: 1, fireball: 1, titan: 1, mage: 1, dragon: 1, gaze: 1 };
...
function handlePointerUp(e) {
    ...
    if (isOverSellZone) {
        ...
        if (sourceType === 'card') {
            ...
            // 将卡牌放回抽牌区并洗牌，然后抽新牌
            const soldCard = state.cards.hand.splice(startIndex, 1)[0];
            state.cards.deck.push(soldCard);
            shuffleDeck();
            drawCard();
        ...
        if (sourceType === 'grid') {
            ...
            if (state.gold >= cost) {
                if (!targetData) {
                    ...
                    const usedCard = state.cards.hand.splice(startIndex, 1)[0];
                    state.cards.deck.push(usedCard);
                    shuffleDeck();
                    drawCard();
                    renderCardHand();
...
function updateCombat(dt) {
    ...
    if (state.battle.phase === 'day') {
        ...
        // 计算当前波的敌人总数：第1波5个，第2波7个，第3波9个...
        const enemiesPerWave = 5 + (state.battle.dayCounter - 1) * 2;
        
        if (state.battle.enemiesSpawnedThisDay >= enemiesPerWave && activeEnemies.length === 0) {
    ...
    // 1. Spawner Logic with cap checks
    state.grid.forEach((b, idx) => {
    ...
    // 2. Spawner (Enemy)
    if (state.battle.phase === 'day' && !state.isChoosingReward) {
        // 计算当前波的敌人总数：第1波5个，第2波7个，第3波9个...
        const enemiesPerWave = 5 + (state.battle.dayCounter - 1) * 2;
        
        if (state.battle.enemiesSpawnedThisDay < enemiesPerWave) {
...
function spawnEnemy() {
    ...
    for (let i = 0; i < count; i++) {
        if (state.battle.enemiesSpawnedThisDay >= enemiesPerWave) break;
...
function showCardTooltip(cardType, cardElement) {
    ...
    DOM.tooltip.innerHTML = `
        ...
        <div class="tooltip-cost">消耗: ${cost} ⚡</div>
        <div class="tooltip-desc">${config.desc}</div>
    `;
    
    // Update quality class
    DOM.tooltip.className = `card-tooltip quality-${config.quality}`;
    ...
    // Calculate position (above the card, centered)

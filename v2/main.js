...
const state = {
    ...
    battle: {
        ...
        enemySpawnTimer: 0,
        phase: 'night', // Start in preparation phase
        dayCounter: 1,
        ...
        damageDealt: {} // Tracked per unit type
    },
    gold: 20,
    maxEnergy: 999,
    ...
    costOffset: 0,
    cards: {
        hand: [null, null, null, null, null], // Initial shop with 5 empty slots (refilled in init)
        next: null
    },
...
const DOM = {
    ...
    refreshShopBtn: document.getElementById('refresh-shop-btn'),
    dpsPanel: document.getElementById('dps-panel'),
    dpsList: document.getElementById('dps-list')
};

function init() {
    createGrid();
    // Initialize mines (no limit anymore)
    state.grid.forEach((b) => {
        if (b && b.type === 'mine') {
            delete b.productionsLeft;
        }
    });

    // Set initial phase UI
    DOM.managementArea.classList.add('preparing');
    DOM.managementArea.classList.remove('fighting');
    initCardSystem();
    setupDragEvents();
    ...
    if (DOM.startWaveBtn) {
        DOM.startWaveBtn.addEventListener('click', startNextWave);
    }
    if (DOM.refreshShopBtn) {
        DOM.refreshShopBtn.addEventListener('click', () => refreshShop(true));
    }
...
function initCardSystem() {
    updateShopUI(); // handles energy display
}
...
function playCard(index) {
    if (state.battle.phase === 'day') return; // Cannot buy during combat
    ...
    // Place building
    const targetSlot = emptySlots.sort((a, b) => a - b)[0];
    ...
    // Remove from shop (DONT refill automatically)
    state.cards.hand[index] = null;
    renderCardHand();
}
...
function renderCardHand() {
    ...
    state.cards.hand.forEach((cardType, idx) => {
        ...
        if (!cardType) {
            cardEl.className = 'card-slot empty';
...
function restartGame() {
    ...
    DOM.startWaveBtn.textContent = "开始战斗";
...
function setupDragEvents() {
    ...
    document.getElementById('action-area').addEventListener('pointerdown', handlePointerDown);
...
function handlePointerDown(e) {
    if (state.battle.phase === 'day') return; // Cannot interact during combat
...
function handlePointerUp(e) {
    ...
    // Handle Selling
    if (isOverSellZone) {
        if (state.battle.phase === 'day') return; // Cannot sell during battle
...
function updateCombat(dt) {
    ...
    if (state.battle.phase === 'day') {
        ...
        if (state.battle.enemiesSpawnedThisDay >= 20 && activeEnemies.length === 0) {
            ...
            let mineBonus = 0;
            state.grid.forEach(b => {
                if (b && b.type === 'mine') mineBonus += (b.level || 1);
            });
            const totalReward = 10 + mineBonus;
            state.gold += totalReward;
            showToast(`战斗结束！获得 ${totalReward} ⚡ (基础10 + 矿工${mineBonus})`, "success");
            highlightEnergyBar();
            updateShopUI();

            // Increment Level
            state.battle.dayCounter++;
            DOM.phaseText.textContent = `🌙 第 ${state.battle.dayCounter} 天 (准备中)`;
            ...
            // Every 3 levels, also show the choice modal
            if ((state.battle.dayCounter - 1) % 3 === 0) {
                state.isChoosingReward = true;
                showNightChoice();
            }
        }
    } else {
        // In Night/Preparation phase, no automatic transition to Day
        if (DOM.startWaveBtn) DOM.startWaveBtn.classList.remove('hidden');
        DOM.phaseText.textContent = `🌙 第 ${state.battle.dayCounter} 天 (准备中)`;
    }

    // Energy Regeneration: REMOVED for auto-chess
    state.energyRegenTimer = 0;

    // Update energy visual display
    if (DOM.goldDisplay) {
        DOM.goldDisplay.textContent = `${state.gold}`;
    }
    ...
    state.grid.forEach((b, idx) => {
        ...
        if (b.type === 'mine') {
            return; // Mines do not tick or produce during combat
        }
        ...
        if (!isAtCapacity) {
            let speed = cellMult * (state.buildingSpeedMult || 1.0);

            // X3 speed for spawners based on user request
            if (['angel', 'titan', 'dragon', 'archer', 'mage'].includes(b.type)) {
                speed *= 3;
            }
    ...
    for (let i = state.battle.entities.length - 1; i >= 0; i--) {
        ...
        if (ent.type === 'enemy') {
            ...
            if (state.battle.phase === 'night') {
                ...
                if (dist > 5) {
                    const moveSpeed = ent.speed * (dt / 1000) * 3; // X3 Return Speed
                    ent.x += nx * moveSpeed;
                    ent.y += ny * moveSpeed;
                }
            } else if (ent.range) {
            ...
            if (dist < 15) {
                ...
                if (ent.isFireball) {
                    ...
                    targets.forEach(t => {
                        ...
                        if (d <= splashRadius) {
                            t.hp -= ent.damage;
                            updateDPS(ent.originType, ent.damage);
                        }
                    });
                    ...
                    if (ent.splashRadius > 0) {
                        ...
                        targets.forEach(t => {
                            const d = Math.hypot(t.x - ent.x, t.y - ent.y);
                            if (d <= ent.splashRadius) {
                                const splashDmg = ent.damage * 0.5;
                                t.hp -= splashDmg;
                                updateDPS(ent.originType, splashDmg);
                            }
                        });
    ...
    for (let a of allies) {
        for (let e of targetEnemies) {
            ...
            if (dist < (a.radius + e.radius)) {
                ...
                e.hp -= a.damage;
                updateDPS(a.originType, a.damage);
                a.hp -= e.damage;
...
function applyRewardEnd() {
    ...
    // In level mode, we stay in night/prep phase until manual start
}

function startNextWave() {
    if (state.battle.phase === 'day') return;
    ...
    // Reset DPS
    state.battle.damageDealt = {};
    updateDPSPanel(); // Clear visuals
    ...
    // UI Phase Switch
    DOM.managementArea.classList.remove('preparing');
    DOM.managementArea.classList.add('fighting');

    showToast(`战斗开始！第 ${state.battle.dayCounter} 天`, "info");
}

function updateDPS(type, damage) {
    if (state.battle.phase !== 'day') return;
    if (!type) return;
    state.battle.damageDealt[type] = (state.battle.damageDealt[type] || 0) + damage;
    updateDPSPanel();
}

function updateDPSPanel() {
    if (!DOM.dpsList) return;

    const damageMap = state.battle.damageDealt;
    const types = Object.keys(damageMap).sort((a, b) => damageMap[b] - damageMap[a]);
    const totalDamage = Object.values(damageMap).reduce((a, b) => a + b, 0);
    const maxDamage = Math.max(...Object.values(damageMap), 1);

    DOM.dpsList.innerHTML = '';

    if (types.length === 0) {
        DOM.dpsList.innerHTML = '<div style="color: #666; font-size: 0.8rem; text-align: center; margin-top: 20px;">等待战斗数据...</div>';
        return;
    }

    types.forEach(type => {
        const dmg = damageMap[type];
        const percent = (dmg / maxDamage) * 100;
        ...
        item.innerHTML = `
            <div class="dps-label">${icon}</div>
            <div class="dps-bar-bg">
                <div class="dps-bar-fill" style="width: ${percent}%"></div>
            </div>
            <div class="dps-val">${Math.floor(dmg)}</div>
        `;
        DOM.dpsList.appendChild(item);
    });
}

function refreshShop(isManual = true) {
    if (isManual) {
        if (state.gold < 2) {
            showToast("能量不足！(需要 2 ⚡)", "error");
            return;
        }
        state.gold -= 2;
        updateShopUI();
    ...
    renderCardHand();
    if (isManual) showToast("商店已刷新", "success");
}
